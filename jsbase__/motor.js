...
async function executar() {
    ...
    if (!codigoOriginal.trim()) {
        ...
        span.innerText = "Digite algum código JSBase para executar...";
        saidaDiv.appendChild(span);
        return;
    }
...
// Atualizar transpilação em tempo real
function atualizarTranspiladoEmTempoReal() {
    const codigoOriginal = editor.value;
    
    if (!codigoOriginal.trim()) {
        if (codigoTranspiladoDiv) {
            codigoTranspiladoDiv.textContent = "// Digite seu código JSBase aqui...";
        }
        return;
    }
    ...
    try {
        ...
        if (codigoTranspiladoDiv) {
            codigoTranspiladoDiv.textContent = "// Erro na transpilação:\n// " + erro.message;
        }
    }
}
...
editor.addEventListener("input", () => {
    updateHighlight();
    atualizarTranspiladoEmTempoReal();
});
