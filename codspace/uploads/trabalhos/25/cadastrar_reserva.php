...
<head>
    ...
    <style>
        ...
        body {
            background:linear-gradient(to right, #e5f087ff, rgba(68, 130, 247, 1));
            font-family: Verdana;
...
<body>
                ...
                <form action="" method="post">
                    ...
                    <?php
                       ...
                       while($computadores = $stmtcomputadores->fetch(PDO::FETCH_ASSOC)){
                        echo "<option value='{$computadores['id']}'>{$computadores['numero']} | {$computadores['processador']} | {$computadores['monitor']} | {$computadores['placa']} | {$computadores['fonte']}</option>";
                        }
                    ?>
                    </select>
 ...
 <section class="resultados">
        <div class="resultado">
            ...
            $sql = "SELECT r.id, a.nome, c.numero, c.processador, c.monitor, c.placa, c.fonte FROM reservas AS r INNER JOIN alunos AS a ON r.id_alunos = a.id INNER JOIN computadores AS c ON r.id_computadores = c.id;
";
