...
if (modoBtn) {
  modoBtn.addEventListener('click', () => {
    ...
    try {
      localStorage.setItem('tema2411', document.body.classList.contains('light-mode') ? 'light' : 'dark');
    } catch {}
  });

  if (localStorage.getItem('tema2411') === 'light') {
...
if (formUser) {
  formUser.addEventListener('submit', e => {
    ...
    window.location.href = 'login2411.html';
  });
}
...
if (formLogin) {
  formLogin.addEventListener('submit', e => {
    ...
    if (email === usuario.email && senha === usuario.senha) {
      ...
      window.location.href = 'perfil2411.html';
    } else {
  ...
  lista.addEventListener('click', e => {
    if (e.target.classList.contains('ver')) {
      ...
      if (item) {
        ...
        window.location.href = 'detalhes2411.html';
      }
    }
  });
}
...
(function () {
  const paginas = ['cadastrar2411.html'];
