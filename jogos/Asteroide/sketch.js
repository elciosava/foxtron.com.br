...
function preload() {
//barulho do lase e dos asteroides explodindo
  for (var i =0; i < 3; i++){
...
var score = 0;
var lives = 3;
var points = [100, 50, 20]; // pontos para os asteroides grandes medios e pequenos
var level = 0;
...
function draw() {
//qaundo a nave bate em um asteroide o jogo reinicia
  ...
  // atualiza a posicao dos laser ao mover a nave
  for(var i = lasers.length - 1; i >= 0; i--) {
    lasers[i].update();
    if(lasers[i].offscreen()) {
      // destruir os laser que sairem da tela
      lasers.splice(i, 1);
    ...
    for (var j = asteroids.length - 1; j >= 0; j--) {
      if (lasers[i].hits(asteroids[j])) {
        // quando o laser bater no asteroide
        ...
        addDust(asteroids[j].pos, dustVel, dustNum);
         // Os novos lasers quebrados de asteróides menores são adicionados à mesma lista
         // de asteróides, para que possam ser referenciados da mesma forma que seus
         // equivalentes de asteróides.
        var newAsteroids = asteroids[j].breakup();
        asteroids = asteroids.concat(newAsteroids);
        // Laser e asteróide anterior são removidos de acordo com as regras do jogo.
