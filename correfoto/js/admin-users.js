...
async function loadAdminUsers(){
  ...
  try{
    ...
    box.innerHTML=d.users.map(u=>`
      <article class="admin-user">
        ...
        ${u.role==='photographer'?`
          ...
          <div class="admin-global-commission"><small>COMISSÃO</small><strong>${Number(u.commission_percent||60)}%</strong><span>Definida globalmente</span></div>`:''}
    ...
    box.querySelectorAll('[data-approve]').forEach(b=>b.onclick=()=>updateUser(b.dataset.approve,{status:'active'}));
    box.querySelectorAll('[data-block]').forEach(b=>b.onclick=()=>updateUser(b.dataset.block,{status:'blocked'}));
    box.querySelectorAll('[data-unblock]').forEach(b=>b.onclick=()=>updateUser(b.dataset.unblock,{status:'active'}));  }catch(e){box.innerHTML=`<div class="empty"><b>${uesc(e.message)}</b></div>`}
}
